    ...
    public ResponseEntity<?> login(@RequestBody UserLoginRequest request) {
        ...
        if (authResponse != null) {
            ...
            if (role == 1) {
                ...
                jsonResponse = "{\"message\": \"Redirect to admin dashboard\", \"user_id\": " + user.getId() +
                        ", \"username\": \"" + user.getUsername() + "\", \"fullnames\": \"" + user.getFullnames() +
                        "\", \"phonenumber\": \"" + user.getPhonenumber() + "\", \"email\": \"" + user.getEmail() + "\"}";
            } else if (role == 2) {
                // Redirect user to user dashboard
                jsonResponse = "{\"message\": \"Redirect to user dashboard\", \"user_id\": " + user.getId() +
                        ", \"username\": \"" + user.getUsername() + "\", \"fullnames\": \"" + user.getFullnames() +
                        "\", \"phonenumber\": \"" + user.getPhonenumber() + "\", \"email\": \"" + user.getEmail() + "\"}";
            } else {
                // Role not recognized
